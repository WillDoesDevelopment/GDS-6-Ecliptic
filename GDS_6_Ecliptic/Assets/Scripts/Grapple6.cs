...
public class Grapple6 : MonoBehaviour
{
    ...
    void MoveTarget()
    {
        ...
        */
        ...
        var forceVec = Vector3.Normalize(playerPoint.position - grapplePoint) * f* Time.deltaTime * 100000f;    //Calculate force
    ...
    public void SnakeActiveFunc()
    {
        ...
        length = Vector3.Distance(handPoint.position, grapplePoint); //Can change to player location (playerPoint.position) if needed
        if (casting == true)
        {
            ...
            {
                //Return
                head.transform.position = Vector3.Lerp(handPoint.position, grapplePoint, (castLength / length) - 0.01f);
                head.transform.LookAt(grapplePoint);

                body.transform.position = Vector3.Lerp(handPoint.position, grapplePoint, (castLength / length / 2f) - 0.01f); ;
                body.transform.localScale = new Vector3(1f, 1f, castLength);
                body.transform.LookAt(grapplePoint);
            ...
            else
            {
                ...
                //t = length / maxLength;
                t = Vector3.Distance(playerPoint.position, grapplePoint) / maxLength;
